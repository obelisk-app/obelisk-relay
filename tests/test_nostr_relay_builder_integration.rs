...
use groups_relay::{
    blacklist::Blacklist, config::Keys, groups::Groups,
    groups_event_processor::GroupsRelayProcessor, whitelist::Whitelist, RelayDatabase,
};
...
async fn test_groups_relay_with_relay_builder() -> anyhow::Result<()> {
    ...
    let groups_processor = GroupsRelayProcessor::new(
        groups,
        keys.public_key(),
